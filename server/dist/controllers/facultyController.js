...
const updateHandled = async (req, res) => {
    const courses = await (0, facultyDataAccess_1.updateHandledCourse)(req.body.data);
    console.log("ak");
    if (!courses)
        return res.status(404).json({ error: 'Failed to retrieve' });
    res.status(200).json(courses);
...
const updateFacultySpecialization = async (req, res) => {
    const programs = await (0, facultyDataAccess_1.updateSpecialization)(req.body.data);
    if (!programs)
        return res.status(404).json({ error: 'Failed to retrieve' });
    res.status(200).json(programs);
};
